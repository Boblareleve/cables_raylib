...
static Rectangle TransformRectangleByCamera2D(Camera2D cam, Rectangle rec); // BOBLARELEVE transform rec by cam to match drawbox and clickbox
...
#if defined(RAYGUI_NO_ICONS)
    result = GuiButton((Camera2D){ .zoom = 1 }, closeButtonRec, "x");
#else
    result = GuiButton((Camera2D){ .zoom = 1 }, closeButtonRec, GuiIconText(ICON_CROSS_SMALL, NULL));
#endif
...
int GuiTabBar(Rectangle bounds, const char **text, int count, int *active)
{
    ...
    for (int i = 0; i < count; i++)
    {
        ...
        if (tabBounds.x < GetScreenWidth())
        {
            ...
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, TEXT_ALIGN_CENTER);
#if defined(RAYGUI_NO_ICONS)
            if (GuiButton((Camera2D){ .zoom = 1 }, RAYGUI_CLITERAL(Rectangle){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 }, "x")) result = i;
#else
            if (GuiButton((Camera2D){ .zoom = 1 }, RAYGUI_CLITERAL(Rectangle){ tabBounds.x + tabBounds.width - 14 - 5, tabBounds.y + 5, 14, 14 }, GuiIconText(ICON_CROSS_SMALL, NULL))) result = i;
#endif
            GuiSetStyle(BUTTON, BORDER_WIDTH, tempBorderWidth);
            GuiSetStyle(BUTTON, TEXT_ALIGNMENT, tempTextAlignment);
        }
...
int GuiButton(Camera2D cam, Rectangle bounds, const char *text)
{
    ...
    if ((state != STATE_DISABLED) && !guiLocked && !guiControlExclusiveMode)
    {
        ...
        if (CheckCollisionPointRec(
            GetMousePosition(), 
            TransformRectangleByCamera2D(cam, bounds)
        )) {
            if (IsMouseButtonDown(MOUSE_LEFT_BUTTON)) state = STATE_PRESSED;
            else state = STATE_FOCUSED;
...
int GuiComboBox(Rectangle bounds, const char *text, int *active)
{
    ...
    GuiButton((Camera2D){ .zoom = 1 }, selector, TextFormat("%i/%i", *active + 1, itemCount));
...
#if defined(RAYGUI_NO_ICONS)
    if (GuiButton((Camera2D){ .zoom = 1 }, leftButtonBound, "<")) tempValue--;
    if (GuiButton((Camera2D){ .zoom = 1 }, rightButtonBound, ">")) tempValue++;
#else
    if (GuiButton((Camera2D){ .zoom = 1 }, leftButtonBound, GuiIconText(ICON_ARROW_LEFT_FILL, NULL))) tempValue--;
    if (GuiButton((Camera2D){ .zoom = 1 }, rightButtonBound, GuiIconText(ICON_ARROW_RIGHT_FILL, NULL))) tempValue++;
#endif
...
int GuiMessageBox(Rectangle bounds, const char *title, const char *message, const char *buttons)
{
    ...
    {
        if (GuiButton((Camera2D){ .zoom = 1 }, buttonBounds, buttonsText[i])) result = i + 1;
